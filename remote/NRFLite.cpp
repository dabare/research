...
uint8_t NRFLite::prepForRx(uint8_t radioId, Bitrates bitrate, uint8_t channel)
{
  ...
  uint8_t address[5] = { 240, 225, 3, 4, radioId };
  //uint8_t address[5] = {210, 240, 240, 240, 240};
...
void NRFLite::prepForTx(uint8_t toRadioId, SendType sendType)
{
  ...
  uint8_t address[5] = { 240, 225, 3, 4, toRadioId };
  //uint8_t address[5] = {225, 240, 240, 240, 240};
...
uint8_t NRFLite::twoPinTransfer(uint8_t data)
{
  ...
  do
  {
    ...
    if (*_momi_PIN & _momi_MASK) {
      byteFromRadio++;  // Read bit from radio on MOMI pin.  If HIGH, set bit position 0 of our byte to 1.
    }
    *_momi_DDR |= _momi_MASK;                         // Change MOMI to be an OUTPUT pin.

    if (data & 0x80) {
      *_momi_PORT |=  _momi_MASK;  // Set MOMI HIGH if bit position 7 of the byte we are sending is 1.
    }
